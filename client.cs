        ...
        for (int i = 0; i < 5; i++)
        {
            using (UdpClient clientS = new UdpClient())
            {
                ...
                while (true)
                {
                    ...
                    Console.WriteLine("5. Client with full access");
                  ...
                  Console.Write("Enter your choice (1, 2, 3, 4, 5, or 6): ");
                    ...
                    {
                        // Code for Client with full access
                        Console.Write("Enter the server IP address to request full access: ");
                        string serverIpAddress = Console.ReadLine();
                        if (serverIpAddress != null)
                        {
                            string requestFullAccessMessage = $"GRANT_FULL_ACCESS:{serverIpAddress}";
                            byte[] data = Encoding.UTF8.GetBytes(requestFullAccessMessage);
                            clientS.Send(data, data.Length, serverName, serverPort);
                        }
                    }
                    else if (choice == "6")
